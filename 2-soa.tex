...
are developed almost every decade. The first generation \textit{1G} launched years later, but
it was only capable of doing voice calls. In 1991, the second generation \textit{2G (GSM)} of 
...
But it was the third generation \textit{3G}, launched in 2001, that enabled new internet-driven
...
streaming into reality. \textit{5G} technologies improves in bandwidth even more and brings 
...
\begin{itemize}
  \item \textbf{\textit{User Equipment (UE)}}
  \item \textbf{\textit{Evolved UMTS Terrestial Radio Access Network (E-UTRAN)}}
