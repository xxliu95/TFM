...
\section{Models}
\label{sec:abrmodels}
This section will go through all the models, classes and helpers in the ABR module.
...
The \texttt{AbrClient} is an implementation of \texttt{ns3::Application}. This class 
receives HTTP-like requests from the \texttt{AbrServer} and sends the requested segment.
...
\begin{figure}[]
  \centering
  \includegraphics[width=\textwidth]{img/abrserver.png}
  \caption{ABR Server.}
  \label{fig:abrserver}
\end{figure}
...
variables. For the segments, a set of random sizes are calculated based on the resolution,
framerate and the encoding bitrate for each representation. There are a predefined set of \texttt{Representations}
that can be changed in the source file.

The possibility of creating a MPD file parser has been considered, 
but it can be done in the future as an improvement.
