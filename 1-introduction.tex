...
relays on the client thus providing better scalability, and there is no need to have 
...
defines the use of \textit{DASH} as the standard continuous delivering of multimedia
...
and defines the \textit{Media Presentation Description (MPD)}, 
...
the \textit{MPD} contains information for each representation such as the
codec, bandwidth, resolution or framerate.
...
on the \textit{Quality of Experience (QoE)}. One problem is that, the algorithm can overestimate
...
\begin{enumerate}
  ...
  \item Implement objective \textit{QoE} and \textit{QoS} metrics.
